...
this.submitted=true
if(!unknown){this.validateExams();exams.forEach(exam=>{examQuestions=[];this.questions.forEach(qst=>{if(qst.examId==exam.examId&&qst.type!==FlowForm.QuestionType.SectionBreak){examQuestions.push(qst);}});if(!generic){let examScore=this.calculateScore(exam,examQuestions);this.scores.push(examScore);}})
