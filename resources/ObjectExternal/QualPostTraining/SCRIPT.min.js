...
output.push(start);}
else{if(generic){start=new FlowForm.QuestionModel({id:'start',title:"Bonjour "+String.fromCodePoint("0x1F44B"),subtitle:"Vous avez suivi une formation sur la plateforme Simplicité et nous aimerions avoir votre avis sur celle-ci. Ces quelques questions on pour but d'améliorer notre processus de formation. Bien entendu, il n'y a pas de bonnes ou de mauvaises réponses "+String.fromCodePoint(0x1F642),description:"D'avance merci pour votre temps !",type:FlowForm.QuestionType.SectionBreak,required:true,examId:exams[0].examId});}
else{start=new FlowForm.QuestionModel({id:'start',tagline:"Bonjour "+String.fromCodePoint("0x1F44B"),title:"Bienvenue sur le portail de questionnaires Simplicité.",subtitle:"Vous allez ici répondre à un certain nombre de questions nous permettant d'évaluer votre niveau de compréhension de la plateforme.",description:"Lorsque vous êtes prêts, cliquez sur 'suivant' ou Appuyez sur la touche ENTRÉE pour commencer.",type:FlowForm.QuestionType.SectionBreak,required:true,});}
...
new Vue({el:'#app',template:qualTemplate,data:function(){return{loading:false,scored:false,submitted:false,completed:false,language:languageParams,questions:output,isValid:!unknown,generic:params.generic,scores:[]}},methods:{onAnswer(qA){let id=qA.id;console.log(id);if(qA.type==FlowForm.QuestionType.SectionBreak&&qA.id.includes("exam")){var usrExObj=app.getBusinessObject("QualUserExam");usrExObj.resetFilters();usrExObj.getForCreate(function(){usrExObj.item.qualUsrexamUsrId=params.userId;usrExObj.item.qualUsrexamExamId=qA.examId;usrExObj.create(function(){exObjRowId=usrExObj.getRowId();usrExObjIds.push(exObjRowId);},usrExObj.item);});}
